...
$(function () {
	...
	$('#send-btn').click(function(){
		...
		$.post(handleUrl,{username:username.val(),content:content.val()},function(data){
			if(data.status){
				if(data.status){
				var str='<dl class="paper a2"><dt><span class="username">'+data.username+'</span><span class="num">No.'+data.id+'</span></dt><dd class="content">'+data.content+'</dd><dd class="bottom"><span class="time">'+data.time+'</span><a href="" class="close"></a></dd></dl>';
				$('#main').append(str);
				$('#close').click();
					}else{
						alert('发布失败');}
